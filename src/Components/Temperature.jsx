...
export default function TemperatureGraph() {
  ...
  const fetchTemperatureData = async () => {
    try {
      ...
      setError("Failed to fetch temperature data. Please try again later.");
    }
  };
  ...
  useEffect(() => {
    ...
    const interval = setInterval(fetchTemperatureData, 60000); // Refetch every 60 seconds
    return () => clearInterval(interval); // Cleanup interval on component unmount
  }, [timeRange]); // Refetch whenever the time range changes

  const isDataEmpty = temperatureData.length === 0;
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow-md">
      ...
      {/* Digital Display for Current Temperature */}
      <div className="flex justify-center items-center mb-6">
        <div className="p-4 bg-green-100 text-green-700 rounded-lg shadow-md w-64 text-center">
      ...
      {error ? (
        ...
        <ResponsiveContainer width="100%" height={400}>
          <LineChart data={temperatureData}>
            ...
            <Line type="monotone" dataKey="temperature" stroke="#34D399" />
          </LineChart>
        </ResponsiveContainer>
      )}
    </div>
