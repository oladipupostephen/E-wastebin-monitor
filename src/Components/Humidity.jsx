...
export default function Humidity() {
    ...
    try {
      ...
      if (data.feeds) {
        ...
        const graphData = filteredData.map((feed) => ({
          ...
          humidity: parseFloat(feed.field2), // Convert humidity to a number
        }));

        setHumidityData(graphData);
        setCurrentHumidity(
          parseFloat(data.feeds[data.feeds.length - 1]?.field2) || null
        ); // Set the latest humidity
      } else {
        setHumidityData([]);
      }
    } catch (err) {
      setError("Failed to fetch humidity data. Please try again later.");
    }
  };

  useEffect(() => {
    fetchHumidityData(); // Initial fetch

    const interval = setInterval(fetchHumidityData, 60000); // Refetch every 60 seconds
    return () => clearInterval(interval); // Cleanup interval on component unmount
  }, [timeRange]); // Refetch whenever the time range changes

  const isDataEmpty = humidityData.length === 0;
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow-md">
      ...
      {/* Digital Display for Current Humidity */}
      <div className="flex justify-center items-center mb-6">
        <div className="p-4 bg-blue-100 text-blue-700 rounded-lg shadow-md w-64 text-center">
          <h2 className="text-lg font-semibold">Current Humidity</h2>
      ...
      {error ? (
        ...
        <ResponsiveContainer width="100%" height={400}>
          <LineChart data={humidityData}>
            ...
            <YAxis
              tick={{ fontSize: 12 }}
              domain={["dataMin - 5", "dataMax + 5"]} // Dynamic Y-axis
              label={{
                value: "Humidity (%)",
                angle: -90,
                position: "insideLeft",
